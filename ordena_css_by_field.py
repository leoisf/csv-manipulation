...
def ordena_csv(nomeArquivo: str, colunaOrdenacao: str, saidaArquivo: int):
    ...
    with open(nomeArquivo, 'r', encoding='utf-8') as arquivo:
        ...
        for i, linha in enumerate(leitor_csv):
            if i == 0:
                cabecalho = linha
                nome_campo = linha['colunaOrdenacao']
                try:
                    colunaOrdenacao = cabecalho.index(nome_campo)
                except ValueError:
                    print(KeyError("Field not found in header."))
            else:
                registros.append(linha)
                #print(linha)

    registros = [r for r in registros if any(campo.strip() for campo in r)]
    registros.sort(key=lambda x: x[colunaOrdenacao] if len(x) > colunaOrdenacao else '')
    ...
    print(f"\n--- Lista ordenada por {cabecalho[colunaOrdenacao]} ---")
